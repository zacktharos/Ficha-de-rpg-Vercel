                    ...
                    <div key={nivel}>
                        ...
                        <div className="space-y-2">
                            {(Array.isArray(habilidades) ? habilidades : []).map(h => {
                                ...
                                let buttonClasses = 'w-full text-left p-3 rounded-lg border transition-all duration-200 ';
                                let buttonText = '';
                                if (isAcquired) {
                                    buttonClasses += 'bg-green-200 border-green-500 text-black cursor-default';
                                    buttonText = 'Adquirida';
                                } else {
                                    buttonClasses += 'bg-white border-gray-300 hover:bg-gray-100 text-black cursor-pointer';
                                    buttonText = almasDisponiveis > 0 ? 'Adquirir com Alma' : `Adquirir (${h.custoPVSemAlma} PV)`;
                                }

                                return (
                                    <button key={h.nome} onClick={() => toggleHabilidade(h)} disabled={isAcquired} className={buttonClasses}>
